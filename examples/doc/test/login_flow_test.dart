...
void main() {
  test('navigation test', () async {
    ...
    await navigTest(
        () => container.navigator.navigate([HomeSegment()]), 'home');

    // navigate to book 3, book 3 needs login => redirected to login page
    await navigTest(
        () => container.navigator.navigate([HomeSegment(), BookSegment(id: 3)]),
        'login;loggedUrl=home%2Fbook%3Bid%3D3;canceledUrl=home');

    // confirm login => redirect to book 3
    await navigTest(
        () => container.navigator.loginScreenOK(), 'home/book;id=3');
    ...
    await navigTest(() => container.navigator.onLogout(),

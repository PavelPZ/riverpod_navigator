...
void main() {
  group('entity', () {
    test('row eTag', () async {
      ...
      for (var i = 0; i < 3; i++) {
        await helper.batchInsertOrMerge('B01', [m1]);
        print(m1.eTag);
      }
      expect(m1 == m1, true);
    });
    ...
    test('with eTag collision', () async {
      ...
      await helper.batchInsertOrMerge('ET', [m1, m2]);
      print(m1.eTag);
      await helper.update(m1);
      print(m1.eTag);
      ...
      try {
        m2.data['p0'] = 'edited';
        await helper.batchInsertOrMerge('ET', [m2, m1Updated]);
      } catch (e) {
        expect(e, 412);
      }
      try {
        await helper.update(m1Updated);
      } catch (e) {
        expect(e, 412);
      }
      expect(0, 0);
    ...
    test('batch update', () async {
      ...
      await helper.insertOrReplace(m1);
      final m2 = RowData.forBatch('B02');
      m1.data['x0'] = 'y00';
      var response = await helper.batchInsertOrReplace('U01', [m1, m2]);
      m1.data['x1'] = 'y11';
      m2.data['x2'] = 'y22';
      response = await helper.batchInsertOrReplace('U01', [m1, m2]);
      response = null;
    ...
    test('batch merge', () async {
      ...
      await helper.insertOrReplace(m1);
      m1.data['x0'] = 'y00';
      var response = await helper.batchInsertOrMerge('B01', [m1, m2]);
      m1.data['x1'] = 'y11';
      m2.data['x2'] = 'y22';
      response = await helper.batchInsertOrMerge('B01', [m1, m2]);
      response = null;
      expect(response, null);
    });
    ...
    test('delete', () async {
      ...
      await helper.insert(model);
      expect(model.eTag!.length > 35, true);
      await helper.delete(model);
    ...
    test('batch delete', () async {
      ...
      await helper.batchDelete('Čau', null);
    });
    ...
    test('insert x merge', () async {
      ...
      await helper.insertOrMerge(model);
      model.data['merged'] = 'merged';
      await helper.merge(model);
      expect(model.eTag!.length > 35, true);
    });
  });
}
